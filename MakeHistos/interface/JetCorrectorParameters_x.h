...
// $Id: JetCorrectorParameters_x.h,v 1.15 2012/03/01 18:24:52 srappocc Exp $
//
// Generic parameters for Jet corrections
//
#ifndef JetCorrectorParameters_x_h
#define JetCorrectorParameters_x_h

#include "H2MuAnalyzer/MakeHistos/interface/Utilities.h"
...
//class JetCorrectorParameters_xHelper;

class JetCorrectorParameters_x 
{
  //---------------- JetCorrectorParameters_x class ----------------
...
//      COND_SERIALIZABLE;
    ...
    //-------- Constructors --------------
    JetCorrectorParameters_x() { valid_ = false;}
    JetCorrectorParameters_x(const std::string& fFile, const std::string& fSection = "");
    JetCorrectorParameters_x(const JetCorrectorParameters_x::Definitions& fDefinitions,
                           const std::vector<JetCorrectorParameters_x::Record>& fRecords) 
  ...
  private:
    //-------- Member variables ----------
    JetCorrectorParameters_x::Definitions                                        mDefinitions;
    std::vector<JetCorrectorParameters_x::Record>                                mRecords;
    bool                                                                       valid_; /// is this a valid set?

//    std::shared_ptr<JetCorrectorParameters_xHelper>                              helper                      COND_TRANSIENT; 
//  COND_SERIALIZABLE;
};
...
class JetCorrectorParameters_xCollection {
  //---------------- JetCorrectorParameters_xCollection class ----------------
  //-- Adds several JetCorrectorParameters_x together by algorithm type ---
 ...
 public:
  ...
  typedef JetCorrectorParameters_x         value_type;
  typedef std::pair<key_type,value_type> pair_type;
  typedef std::vector<pair_type>         collection_type;


  // Constructor... initialize all three vectors to zero
  JetCorrectorParameters_xCollection() { corrections_.clear(); correctionsL5_.clear(); correctionsL7_.clear(); }
  ...
  JetCorrectorParameters_x const & operator[]( key_type k ) const;

  // Access the JetCorrectorParameter via a string. 
  // Will find the hashed value for the label, and call via that 
  // operator. 
  JetCorrectorParameters_x const & operator[]( std::string const & label ) const {
 ...
 protected:
  ...
  friend struct                          JetCorrectorParameters_xInitializeTransients;
};

struct JetCorrectorParameters_xInitializeTransients {
  void operator()(JetCorrectorParameters_xCollection& jcpc) {
