...
// $Id: JetCorrectorParameters_x.cc,v 1.20 2012/03/01 18:24:53 srappocc Exp $
//
// Generic parameters for Jet corrections
//
#include "H2MuAnalyzer/MakeHistos/interface/JetCorrectorParameters_x.h"
//#include "CondFormats/JetMETObjects/interface/JetCorrectorParameters_xHelper.h"
#include <iostream>
#include <iomanip>
#include <fstream>
#include <sstream>
...
//------------------------------------------------------------------------
//--- JetCorrectorParameters_x::Definitions constructor --------------------
//--- takes specific arguments for the member variables ------------------
//------------------------------------------------------------------------
JetCorrectorParameters_x::Definitions::Definitions(const std::vector<std::string>& fBinVar, const std::vector<std::string>& fParVar, const std::string& fFormula, bool fIsResponse)
{
...
//------------------------------------------------------------------------
JetCorrectorParameters_x::Definitions::Definitions(const std::string& fLine)
{
      ...
      if (tokens.size() < 6)
        {
          ...
          handleError("JetCorrectorParameters_x::Definitions",sserr.str());
        }
...
//------------------------------------------------------------------------
//--- JetCorrectorParameters_x::Record constructor -------------------------
//--- reads the member variables from a string ---------------------------
//------------------------------------------------------------------------
JetCorrectorParameters_x::Record::Record(const std::string& fLine,unsigned fNvar) : mMin(0),mMax(0)
{
  mNvar = fNvar;
      ...
      if (tokens.size() < 3)
        {
          ...
          handleError("JetCorrectorParameters_x::Record",sserr.str());
        }
...
std::ostream& operator<<(std::ostream& out, const JetCorrectorParameters_x::Record& fBin)
{
...
//------------------------------------------------------------------------
//--- JetCorrectorParameters_x constructor ---------------------------------
//--- reads the member variables from a string ---------------------------
//------------------------------------------------------------------------
JetCorrectorParameters_x::JetCorrectorParameters_x(const std::string& fFile, const std::string& fSection)
{
  ...
  if (currentDefinitions=="")
    handleError("JetCorrectorParameters_x","No definitions found!!!");
  ...
  if (mRecords.empty() && currentSection != "")
    {
      ...
      handleError("JetCorrectorParameters_x",sserr.str());
    }
...
//------------------------------------------------------------------------
//--- initializes the correct JetCorrectorParameters_xHelper ---------------
//------------------------------------------------------------------------
void JetCorrectorParameters_x::init()
{
  std::sort(mRecords.begin(), mRecords.end());
//  helper = std::make_shared<JetCorrectorParameters_xHelper>();
//  helper->init(mDefinitions,mRecords);
} 
//------------------------------------------------------------------------
//--- returns the index of the record defined by fX ----------------------
//------------------------------------------------------------------------
int JetCorrectorParameters_x::binIndex(const std::vector<float>& fX) const
{
  int result = -1;
...
//------------------------------------------------------------------------
int JetCorrectorParameters_x::binIndexN(const std::vector<float>& fX) const
{
...
//------------------------------------------------------------------------
int JetCorrectorParameters_x::neighbourBin(unsigned fIndex, unsigned fVar, bool fNext) const
{
  int result = -1;
  unsigned N = mDefinitions.nBinVar();
  if (fVar >= N)
...
//------------------------------------------------------------------------
unsigned JetCorrectorParameters_x::size(unsigned fVar) const
{
...
//------------------------------------------------------------------------
std::vector<float> JetCorrectorParameters_x::binCenters(unsigned fVar) const
{
...
//------------------------------------------------------------------------
void JetCorrectorParameters_x::printScreen() const
{
  std::cout<<"--------------------------------------------"<<std::endl;
...
//------------------------------------------------------------------------
void JetCorrectorParameters_x::printFile(const std::string& fFileName) const
{
...
std::string
JetCorrectorParameters_xCollection::findLabel( key_type k ){
...
std::string
JetCorrectorParameters_xCollection::findL5Flavor( key_type k ){
...
std::string
JetCorrectorParameters_xCollection::findL7Parton( key_type k ){
  if ( k == L7Parton ) return labels_[L7Parton];
  else
    return l7Partons_[k / 1000 - 1];
}

void JetCorrectorParameters_xCollection::getSections(std::string inputFile,
...
void JetCorrectorParameters_xCollection::push_back( key_type i, value_type const & j, label_type const & flav) {
  ...
  if ( isL5(i) ) {
    ...
    std::cout << "***** NOT ADDING " << flav << ", corresponding position in JetCorrectorParameters_x is not found." << std::endl;
  }
}
...
JetCorrectorParameters_x const & JetCorrectorParameters_xCollection::operator[]( key_type k ) const {
...
void JetCorrectorParameters_xCollection::validKeys(std::vector<key_type> & keys ) const {
  keys.clear();
...
JetCorrectorParameters_xCollection::key_type
JetCorrectorParameters_xCollection::getL5Bin( std::string const & flav ){
...
JetCorrectorParameters_xCollection::key_type
JetCorrectorParameters_xCollection::getL7Bin( std::string const & flav ){
  std::vector<std::string>::const_iterator found =
...
bool JetCorrectorParameters_xCollection::isL5( key_type k ) {
  return k == L5Flavor ||
    ( k / 100 > 0 && k / 1000 == 0 );
}
// Check if this is an L7 hashed value
bool JetCorrectorParameters_xCollection::isL7( key_type k ) {
  return k == L7Parton ||
    ( k / 1000 > 0 );
}
...
JetCorrectorParameters_xCollection::key_type
JetCorrectorParameters_xCollection::findKey( std::string const & label ) const {
