...
const TString IN_DIR   = "/eos/cms/store/group/phys_higgs/HiggsExo/H2Mu/UF/ntuples/2017/94X_v4/prod-v17.2.0.skim3l/DYJetsToLL_M-50_TuneCP5_13TeV-amcatnloFXFX-pythia8/ZJets_AMC/191212_101320";
...
// const TString SAMPLE   = "custom_ggH";
...
//const TString OUT_DIR = "/afs/cern.ch/work/e/eyigitba/public/H2Mu/2017/Histograms";
...
void d0VsPtStudies2D( TString sample = "", TString in_dir = "", TString out_dir = "",
  ...
  if (in_files.size() == 0) {
    for (int i = MIN_FILE; i <= MAX_FILE; i++) {
      in_file_name.Form("%s/NTuple_%d.root", in_dir.Data(), i);
      std::cout << "Adding file " << in_file_name.Data() << std::endl;
...
//            float mu_pt_BSRoch  = (mu.pt_bs > 0 ? (mu.pt_bs * mu.pt_Roch / mu.pt) : mu.pt_Roch);
            ...
            if (nVtx[0] <= nVertices && nVertices <= nVtx[1]){
              ...
              // BookAndFill( h_map_2D, h_pre+"dRelPt2p0_BS_vs_d0_BS", 400, -0.01,  0.01, 4000,    -40, 40, mu.d0_BS*mu.charge, 10000*(mu.pt_bs      - genMu.pt) / pow(genMu.pt, 2.0),event_wgt );
              // BookAndFill( h_map_2D, h_pre+"dRelPt2p0_BSRoch_vs_d0_BS", 400, -0.01,  0.01, 4000,    -40, 40, mu.d0_BS*mu.charge, 10000*(mu_pt_BSRoch      - genMu.pt) / pow(genMu.pt, 2.0),event_wgt );
        ...
        if (ETA_CUTS.at(iEta).find("eta_inc") != string::npos){
          // Corrections derived from DY MC 
          float mu1_ptCorr = 0.0;
          float mu2_ptCorr = 0.0;
          if ( fabs(mu1.eta) < 0.9 ) {
            if ( YEAR == "2016" ) mu1_ptCorr = mu1.pt_Roch - 411.343/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge ;
            if ( YEAR == "2017" ) mu1_ptCorr = mu1.pt_Roch - 650.839/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge ;
            if ( YEAR == "2018" ) mu1_ptCorr = mu1.pt_Roch - 582.32/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge;
          }
          else if ( 0.9 <= fabs(mu1.eta) && fabs(mu1.eta) < 1.7){
            if ( YEAR == "2016" ) mu1_ptCorr = mu1.pt_Roch - 673.398/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge;
            if ( YEAR == "2017" ) mu1_ptCorr = mu1.pt_Roch - 988.369/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge;
            if ( YEAR == "2018" ) mu1_ptCorr = mu1.pt_Roch - 974.047/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge;
          }
          else if ( 1.7 <= fabs(mu1.eta) ){
            if ( YEAR == "2016" ) mu1_ptCorr = mu1.pt_Roch - 1098.984/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge;
            if ( YEAR == "2017" ) mu1_ptCorr = mu1.pt_Roch - 1484.616/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge;
            if ( YEAR == "2018" ) mu1_ptCorr = mu1.pt_Roch - 1263.388/10000 * mu1.pt_Roch * mu1.pt_Roch * mu1.d0_BS * mu1.charge;
          }

          if ( fabs(mu2.eta) < 0.9 ) {
            if ( YEAR == "2016" ) mu2_ptCorr = mu2.pt_Roch - 411.343/10000 * mu2.pt_Roch * mu2.pt_Roch * mu2.d0_BS * mu2.charge ;
            if ( YEAR == "2017" ) mu2_ptCorr = mu2.pt_Roch - 650.839/10000 * mu2.pt_Roch * mu2.pt_Roch * mu2.d0_BS * mu2.charge ;
            if ( YEAR == "2018" ) mu2_ptCorr = mu2.pt_Roch - 582.32/10000 * mu2.pt_Roch * mu2.pt_Roch * mu2.d0_BS * mu2.charge;
          }
          if ( 0.9 <= fabs(mu2.eta) && fabs(mu2.eta) < 1.7){
            if ( YEAR == "2016" ) mu2_ptCorr = mu2.pt_Roch - 673.398/10000 * mu2.pt_Roch * mu2.pt_Roch * mu2.d0_BS * mu2.charge;
            if ( YEAR == "2017" ) mu2_ptCorr = mu2.pt_Roch - 988.369/10000 * mu2.pt_Roch * mu2.pt_Roch * mu2.d0_BS * mu2.charge;
            if ( YEAR == "2018" ) mu2_ptCorr = mu2.pt_Roch - 974.047/10000 * mu2.pt_Roch * mu2.pt_Roch * mu2.d0_BS * mu2.charge;
          }
          if ( 1.7 <= fabs(mu2.eta) ){
            if ( YEAR == "2016" ) mu2_ptCorr = mu2.pt_Roch - 1098.984/10000 * mu2.pt_Roch * mu2.pt_Roch * mu2.d0_BS * mu2.charge;
          ...
          mu1_vec_Roch.SetPtEtaPhiM(mu1.pt_Roch,mu1.eta,mu1.phi,0.105658367);
          mu2_vec_Roch.SetPtEtaPhiM(mu2.pt_Roch,mu2.eta,mu2.phi,0.105658367);
          dimu_vec_Roch = mu1_vec_Roch + mu2_vec_Roch;

          mu1_vec_corr.SetPtEtaPhiM(mu1_ptCorr,mu1.eta,mu1.phi,0.105658367);
          mu2_vec_corr.SetPtEtaPhiM(mu2_ptCorr,mu2.eta,mu2.phi,0.105658367);
          dimu_vec_corr = mu1_vec_corr + mu2_vec_corr;

                      
//          float dimu_mass_BSRoch  = (dimu.mass_bs>0 ? (dimu.mass_bs*dimu.mass_Roch/dimu.mass):dimu.mass_Roch);
          ...
          // BookAndFill( h_map_2D, h_pre+"dimu_mass_BS_vs_d0_BS", 400, -0.01,  0.01, 120,    100, 160, mu1.d0_BS*mu1.charge+mu2.d0_BS*mu2.charge, dimu.mass_bs,event_wgt );
          ...
          // BookAndFill( h_map_2D, h_pre+"dimu_mass_BS_vs_d0_BS_muP", 400, -0.01,  0.01, 120,    100, 160, mu_d0_BS, dimu.mass_bs,event_wgt );
          // BookAndFill( h_map_2D, h_pre+"dimu_mass_BSRoch_vs_d0_BS_muP", 400, -0.01,  0.01, 120,    100, 160, mu_d0_BS, dimu_mass_BSRoch,event_wgt );
